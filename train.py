...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    input = torch.zeros((1, 3, 512, 512)).to(device)
    flops, params = profile(model.to(device), inputs=(input,))

    print("参数量：", params / 10 ** 6)
    ...
    if RANK in [-1, 0]:
        val_loader = create_dataloader(val_path, imgsz, batch_size // WORLD_SIZE * 2, gs, single_cls,
                                       ...
                                       rect=True, rank=-1, workers=workers * 2, pad=0,
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            #if imgs.shape[2]!=256 or imgs.shape[2]!=256:
                #continue
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='models/yolov5-pac.yaml', help='model.yjizongaml path')
